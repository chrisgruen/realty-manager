...
class XmlConverter
{
    ...
    private function fixHouseType() {
        ...
        if($this->importedData['house_type']=='Wohnung') {
            $this->addImportedData('house_type','Mietwohnung');
            $this->addImportedData('apartment_type', (floor($this->importedData['number_of_rooms']).'-Raum-Wohnung'));
        }
    }
    ...
    private function fetchCategoryAttributes()
    {
        ...
        if (!empty($objectTypeAttributes)) {
            if (isset($objectTypeAttributes['kauf']) && $this->isBooleanLikeStringTrue($objectTypeAttributes['kauf'])) {
                $this->addImportedData('object_type', Objectimmo::TYPE_FOR_SALE);
            } else {
                $this->addImportedData('object_type', Objectimmo::TYPE_FOR_RENT);
            }
        }
    ...
    private function fetchStatus()
    {
        ...
        ? Objectimmo::STATUS_RENTED : Objectimmo::STATUS_VACANT;
    ...
    private function fetchEnergyCertificateType()
    {
        ...
        $validValues = [
            'BEDARF' => Objectimmo::ENERGY_CERTIFICATE_TYPE_REQUIREMENT,
            'VERBRAUCH' => Objectimmo::ENERGY_CERTIFICATE_TYPE_CONSUMPTION,
        ];
    ...
    private function fetchEnergyCertificateYear()
    {
        ...
        $validValues = [
            '2008' => Objectimmo::ENERGY_CERTIFICATE_YEAR_2008,
            '2014' => Objectimmo::ENERGY_CERTIFICATE_YEAR_2014,
            'ohne' => Objectimmo::ENERGY_CERTIFICATE_YEAR_NOT_AVAILABLE,
            'nicht_noetig' => Objectimmo::ENERGY_CERTIFICATE_YEAR_NOT_REQUIRED,
        ];
    ...
    private function fetchBuildingType()
    {
        ...
        $validValues = [
            'wohn' => Objectimmo::BUILDING_TYPE_RESIDENTIAL,
            'nichtwohn' => Objectimmo::BUILDING_TYPE_BUSINESS,
        ];
